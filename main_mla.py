...
from optim.train_mla import train_base
#from models.base_mla import ModelArgs, Transformer
# from models.skip_mla import ModelArgs, Transformer
from models.base_gqa import ModelArgs, Transformer
#from models.skip_gqa import ModelArgs, Transformer
# from models.base_llama import ModelArgs, Transformer
# from models.skip_llama import ModelArgs, Transformer
import distributed
...
def main(args): 
    ...
    for group in group_specs:
        if "lr" in group and group["lr"] == 1.0:  # 检查是否是 theta 参数组
            group["lr"] = args.lr * 1  # 根据需要调整学习率
        ...
        if args.scheduler != 'none':
            if args.scheduler in ['cos', 'linear']:
                scheduler = torch.optim.lr_scheduler.OneCycleLR(
                    optimizer=opt,
                    max_lr=args.lr,
                    total_steps=args.iterations,  # 使用原始总步数
...
if __name__ == "__main__":
    args = get_args()
    print(f"Using seed: {args.seed}")  # 打印seed
